/**
 * Test Customer Reminder Fields - Automatic Login
 *
 * Logs in first to get a valid token, then tests reminder fields
 */
...
async function test() {
  ...
  try {
    ...
    const loginResponse = await axios.post(
      `${BASE_URL}/tenants/${TENANT_ID}/login`,
      LOGIN
    );
    ...
    if (customer.reminder_opt_in === true) {
      ...
      throw new Error(`reminder_opt_in is ${customer.reminder_opt_in}, expected true`);
    }

    if (customer.reminder_preference === 'email') {
      console.log('✓ reminder_preference saved correctly (email)');
    } else {
      throw new Error(`reminder_preference is ${customer.reminder_preference}, expected email`);
    }
    console.log();
    ...
    if (updatedCustomer.reminder_opt_in === false) {
      ...
      throw new Error(`reminder_opt_in is ${updatedCustomer.reminder_opt_in}, expected false`);
    }

    if (updatedCustomer.reminder_preference === 'none') {
      console.log('✓ reminder_preference updated correctly (none)');
    } else {
      throw new Error(`reminder_preference is ${updatedCustomer.reminder_preference}, expected none`);
    }
    console.log();
    ...
    customerId2 = createResponse2.data.id;
    ...
    if (defaultCustomer.reminder_opt_in === true) {
      ...
      throw new Error(`reminder_opt_in is ${defaultCustomer.reminder_opt_in}, expected true`);
    }

    if (defaultCustomer.reminder_preference === 'sms') {
      console.log('✓ reminder_preference defaults to sms');
    } else {
      throw new Error(`reminder_preference is ${defaultCustomer.reminder_preference}, expected sms`);
    }
    console.log();
    ...
    if (token) {
      ...
      if (customerId2) {
        try {
          ...
          console.log(`Cleaned up customer ${customerId2}`);
        } catch (e) {
          // Ignore cleanup errors
        }
      }
