...
export const tenantApi = {
   ...
   * Create a new tenant
   */
  createTenant: async (data: CreateTenantDto): Promise<{
    tenant_id: number;
    company_name: string;
    subdomain: string;
    message: string;
  }> => {
    const response = await platformAdminClient.post('/platform-admin/tenants', data);
    return response.data;
  },
   ...
   */
  deleteTenant: async (tenantId: number): Promise<{
    message: string;
    tenantId: number;
    companyName: string;
  }> => {
   ...
   * Reset user password
   */
  resetUserPassword: async (tenantId: number, userId: number): Promise<{
    username: string;
